...
impl AnalyzerEventSubscriberUdpMetric {
    ...
    pub async fn run(mut self) {
        let mut sock = UdpSocket::bind("0.0.0.0:0").await.expect("could not register Metric Udp Socker. should never happen.");
        ...
        loop {
            match self.broadcast_recv.recv().await {
                ...
                Ok(event) => match event {
                    BroadcastEvent::UdpEchoAnalyzerEventServer(e) => match self.on_udp_echo_analyzer_event_server(&mut sock, e).await {
                        Ok(_) => {}
                        Err(e) => {
    ...
    pub async fn on_udp_echo_analyzer_event_server(&self, sock: &mut UdpSocket, event: UdpEchoAnalyzerEventServer) -> Result<(), ::anyhow::Error> {
    ...
    pub fn on_udp_echo_analyzer_event_datacenter(&self, _sock: &mut UdpSocket, _event: UdpEchoAnalyzerEventDatacenter) {
    }
}
